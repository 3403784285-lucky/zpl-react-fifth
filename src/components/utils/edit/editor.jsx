...
if (visible) {
  ...
  if (res.code === 200) {
    ...
    if (res.data && res.data.res) {
      console.log("结果：", res.data.res);
      try {
        // 使用 eval 将字符串转换为对象
        const option = eval('(' + res.data.res + ')');
        setOption(option);
  ...
  const polishClick = async (value) => {
    ...
    try {
      const response = await fetch(AI_SSE_URL, {
        method: 'POST',
        body: JSON.stringify({type:"polish",userQuery:selectedText+value}),
        headers: {
      ...
      while (true) {
        ...
        // chunk 是本次读取到的 SSE 内容
        let chunk = decoder.decode(value, { stream: true });

        // 将 chunk 按照换行符拆分，因为 SSE 里每一行通常一个事件
        ...
        for (let line of lines) {
          ...
          // 去掉 'data:' 前缀
          let jsonStr = line.substring(5).trim(); // 例如 {"token":"明"}

          // 如果服务端约定了 '[DONE]' 表示流式结束，可以在这里提前 break
          if (jsonStr === '[DONE]') {
            break;
          }
  ...
  const rewriteClick = async () => {
    ...
    try {
      const response = await fetch(AI_SSE_URL, {
        method: 'POST',
        body: JSON.stringify({type:"rewriting",userQuery:selectedText}),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'text/event-stream',
        },
  ...
  const correctionClick = async () => {
    ...
    try {
      const response = await fetch(AI_SSE_URL, {
        method: 'POST',
        body: JSON.stringify({type:"correction",userQuery:selectedText}),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'text/event-stream',
        },
  ...
  const continuationClick = async () => {
    const selectedText = editor.state.doc.cut(editor.state.selection.from, editor.state.selection.to).textContent;
    const res=await aiFun.deduct("文本续写")
    if(res.code==200) {
      // 显示生成中的模态框，使用CSS动画来模拟生成中的效果
      const generatingModal = Modal.info({
        title: '生成中',
  ...
  const expansionClick = async () => {
    ...
    try {
      const response = await fetch(AI_SSE_URL, {
        method: 'POST',
        body: JSON.stringify({type:"expansion",userQuery:selectedText}),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'text/event-stream',
        },
  ...
  const summarizationClick = async () => {
    ...
    try {
      const response = await fetch(AI_SSE_URL, {
        method: 'POST',
        body: JSON.stringify({type:"summarization",userQuery:selectedText}),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'text/event-stream',
        },
  ...
  const translateClick = async () => {
    ...
    try {
      const response = await fetch(AI_SSE_URL, {
        method: 'POST',
        body: JSON.stringify({type:"translation",userQuery:selectedText}),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'text/event-stream',
        },
