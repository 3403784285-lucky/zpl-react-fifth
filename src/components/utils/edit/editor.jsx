...
if (visible) {
  ...
  const polishClick = async (value) => {
    ...
    try {
      const response = await fetch('http://192.168.43.89:5000/textBeautification', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const rewriteClick = async () => {
    ...
    try {
      const response = await fetch('http://192.168.43.89:5000/rewrite', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': 'your-authorization-token',
  ...
  const correctionClick = async () => {
    ...
    try {
      const response = await fetch('http://192.168.43.89:5000/textErrorCorrection', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': 'your-authorization-token',
  ...
  const continuationClick = async () => {
    ...
    try {
      const response = await fetch('http://192.168.50.41:8090/ai/sse', {
        method: 'POST',
        body: JSON.stringify({type:"continuation",userQuery:selectedText}),
        headers: {
      ...
      while (true) {
        ...
        result1 += decoder.decode(value, { stream: true });
        setResult(JSON.parse(result1)); // 逐步更新状态，以动态显示在模态框中
  ...
  const expansionClick = async () => {
    ...
    try {
      const response = await fetch('http://192.168.43.89:5000/expansion', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': 'your-authorization-token',
  ...
  const summarizationClick = async () => {
    ...
    try {
      const response = await fetch('http://192.168.43.89:5000/abbreviation', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': 'your-authorization-token',
  ...
  const translateClick = async () => {
    ...
    try {
      const response = await fetch('http://192.168.43.89:5000/textTranslation', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': 'your-authorization-token',
        ...
        <Content className='flex  h-full w-full '>
          ...
          <div className="bg-color-white flex-1">
            ...
            {current == 1 ? (<Extraction editor={editor} />) : (current == 2 ? (<Dictionary />) : (current == 3 ? (<Generation />) : (current == 4 ? (<Polishing editor={editor} fontFamilyH1Value={fontFamilyH1Value}
              ...
              setLineHeightH4Value={setLineHeightH4Value} />) : (current == 5 ? (<MyExample editor={editor} />) : (current == 6 ? (<AiHelper />) : (current == 7 ? (<PassageComment editor={editor} />) : (current==8?(<Picture editor={editor}/>):(<Style  
                handleApplyStyles={handleApplyStyles}
                ...
                setLineHeightH3Value={setLineHeightH3Value}
                lineHeightH4Value={lineHeightH4Value}
                setLineHeightH4Value={setLineHeightH4Value} />))))))))}
          </div>
        </Content>
      </div>
    </>
