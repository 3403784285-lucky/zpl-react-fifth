...
const itemTwosCopy = [
  ...
  getItem('sys-manage', <BookOutlined />, '系统通知管理'),
  getItem('order-manage', <SnippetsOutlined />, '订单管理'),
  getItem('vector-manage', <CaretUpOutlined />, '向量数据库'),
    ...
    <Layout >
      <Header className='p-10' style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'white' }} >
        <div className="pre-frame flex-r-start-end">
          ...
          {status=="admin"? (<img src="/img/back-logo.png" style={{ width: '85px', height: '28px' }} />) :(<img src="/img/logo.png" style={{ width: '85px', height: '28px' }} />)}
          {status == "admin" || <Search className='m-l-80' placeholder="请输入关键字"
            onChange={(e) => toSmall(e.target.value)} style={{ width: '300px' }} onClick={onSearch} enterButton />}
        </div>
        <div className="after-part m-r-20 flex">
          {status=="admin" ?'': <Button className=' bg-color-second text-color-white b-rd-10' onClick={() => navigate("/member-center")}>会员中心</Button>}
          <IconFont type="icon-icon-yichang "  onClick={() => navigate("/information-notice")} className='font-size-lg m-x-20' />
          <Popover placement="bottom"
            content={<div>
              {status == "admin" || <><div className='b-rd-6 flex hover-effect p-6' onClick={() => navigate('/person')}> <HomeOutlined className='m-x-6' /><div >个人中心</div></div>
          ...
          >
            <img src={img ?? storage.getItem("user")?.avatar ?? ''} disabled={status !== "admin"} style={{ width: '35px', height: '35px', borderRadius: '50%' }} />
          </Popover>
        </div>
      </Header>
      <Layout>
