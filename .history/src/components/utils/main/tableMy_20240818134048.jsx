...
function TableMy({ getFun, dataContent, deleteFun, deleteBatchFun, uniqueText }) {
    ...
    const [selectedDocument, setSelectedDocument] = useState(null); // 新增选中文档的id状态
    const [selectedValue, setSelectedValue] = useState('可编辑');
    const [selectedValueCopy, setSelectedValueCopy] = useState(0);
    ...
    const handleOk1 = async () => {
        if (selectedFolder) {
            ...
            const res = await foldersFun.moveDocumentToFolder({ documentId: selectedDocument.id, folderId: selectedFolder.folderId });
            if (res.code == 200) {
                message.success('添加到文件夹成功');
            }
    ...
    const handleDeleteConfirm = async () => {
        ...
        let res = null;
        if (isFolder) {
            res = await foldersFun.deleteFolderCopy(storage.getItem("folder").folderId, selectedDocumentId)

        } else {
            res = await deleteFun(selectedDocumentId);
        }
        if (res.code == 200) {
    ...
    const fetchData = async (editPermission,validDays) => {
        const res = await shareCFun.document({ id:selectedDocumentId, editPermission:editPermission, validDays:validDays })
        if (res.code == 200) {
    ...
    const handleChange = (e) => {
        setSelectedValue(e.value) // { value: "lucy", key: "lucy", label: "Lucy (101)" }
        fetchData(e.value,selectedValueCopy)
    };
    const handleChangeCopy = (e) => {
        setSelectedValueCopy(e.value)
        fetchData(selectedValue,e.value)
    ...
    const start = async (e) => {
        ...
        if (deleteBatchFun) {
            let res = null;
            if (isFolder) {
                res = await foldersFun.deleteDocumentByFolderId({ folderId: storage.getItem("folder").folderId, documentIds: selectedRowKeys });

            } else {
                res = await deleteBatchFun(selectedRowKeys);
    ...
    const handleHistoryClick = async (record) => {
        try {
            ...
            Modal.confirm({
                ...
                content: (
                    <div className='m-t-10 p-10' style={{ width: '700px', height: '400px' }}>  <RichTextEditor setMark={setMark} toDocument={handleModalTo} documentId={record.id} /></div>
            ...
            <List
                ...
                renderItem={(item) => (<>
                    <List.Item
                        ...
                        className={selectedFolder === item ? 'is-selected p-10 ' : 'p-10'}
