...
import { generate } from 'markmap-lib';

import { create as createMarkmap } from 'markmap-view';
...
const handleMarkdownChange = (markdownContent) => {
  const { root } = generate(document.getElementById('markmap-container'), markdownContent);

  // 渲染markmap
  MarkmapView.create(root, '#markmap-container');
};
...
const Tiptap = () => {
  ...
  return <>
    ...
    />
     <div id="markmap-container" style={{ height: '400px' }}></div>
    <EditorContent className={`p-24 ${classes.codeBlock} `} editor={editor} onChange={handleMarkdownChange}/>
