...
const handleMarkdownChange = (content) => {
  ...
  const { root, features } = transformer.transform("# My map");

  // 获取所需的样式和脚本
  const { styles, scripts } = transformer.getUsedAssets("# My map");
...
const Tiptap = () => {
  ...
  return <>
    ...
    />
     <div id="markmap-container" style={{ height: '400px' }} onClick={handleMarkdownChange}></div>
