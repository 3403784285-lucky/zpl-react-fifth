...
const Tiptap = ({ user, room, content, editor = {} }) => {
  ...
  const [fontSizeH1Value, setFontSizeH1Value] = useState('16px'); // Default font size
  const [fontSizeH2Value, setFontSizeH2Value] = useState('16px'); // Default font size
  const [fontSizeH3Value, setFontSizeH3Value] = useState('16px'); // Default font size
  const [fontSizeH4Value, setFontSizeH4Value] = useState('16px'); // Default font size
...
`,
  ...
  const handleApplyStyles = () => {
    ...
    //     // 如果是级别为 1 的标题节点，则将其位置添加到数组中
    //     heading1Positions.push({ from: pos, to: pos + node.nodeSize });
    //     console.log(pos,pos+node.nodeSize)
    //   }
    // });

    // // 添加 mark 来设置字体大小
    // heading1Positions.forEach(({ from, to }) => {
    //   editor.chain().focus(from).setMark('textStyle', { fontSize: '18px' }).run();
    // });
    editor.chain( ).setTextSelection({ from: 0, to: 8 }).setFontSize(19).run();
  ...
  return (<>{spin ? (<Spin size="large" style={{ margin: '300px 520px' }} />) : (<div style={{ height: '100%' }}>
    ...
    ) : (<>  <div style={{ width: `${shrink ? 100 : 70}%`, overflowY: 'auto' }}>
      <EditorContent className={`p-24 ${classes.codeBlock}`} editor={editor} />
    </div>
      <div className={`position-relative ${shrink ? 'show-no' : ''}`} style={{ width: '30%', backgroundColor: 'red', boxShadow: 'rgba(50, 50, 93, 0.25) 0px 2px 5px -1px, rgba(0, 0, 0, 0.3) 0px 1px 3px -1px' }}>
        <CaretRightOutlined className='position-absolute font-size-mlg' onClick={toggleSize} style={{ left: 20, bottom: 10 }} />
        <Content className='flex  h-full w-full '>
          ...
          <div className="bg-color-white flex-1">
            ...
            {current == 1 ? (<Extraction />) : (current == 2 ? (<Dictionary />) : (current == 3 ? (<Generation />) : (current==4?(<Polishing fontFamilyH1Value={fontFamilyH1Value}
              ...
              setLineHeightH4Value={setLineHeightH4Value} />):(current==5?(<MyExample/>):(current==6?(<AiHelper/>):(<PassageComment/>))))))}
          </div>
        </Content>
      </div>
    </>
