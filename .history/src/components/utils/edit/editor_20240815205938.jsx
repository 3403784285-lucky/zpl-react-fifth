...
const request = extend({
  prefix: 'http://192.168.50.41:8090',
  timeout: 300000,
  errorHandler,
});
...
const Tiptap = ({ setIsModalVisible,content, editor = {}, provider, saveTimeInit }) => {
  ...
  const continuationClick = async () => {
    ...
    const formData = new FormData();
    formData.append('text', selectedText);
    formData.append('passage', editor.getText());
    const response = await request('/api/your-endpoint', {
      method: 'POST',
      data: formData,
      responseType: 'blob', // 设置流式响应
      headers: {
        'Content-Type': 'multipart/form-data', // 指定请求为表单数据
      },
    });
    // 关闭生成中的模态框
    generatingModal.destroy();
