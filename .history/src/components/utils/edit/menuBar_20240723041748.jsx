  ...
  return (
    <div className={`upper-frame bg-color-blue flex-center-center shadow`} style={{ height: '8%' }}>
      ...
      <Tooltip placement="bottom" title={"行距"} arrow={mergedArrow}>
        <Popover placement="bottom" content={icons.map((icon, index) => {if(index==0) return <IconFont type={`icon-${icon}`} key={icon} onClick={() => hide(index)} className={`m-6 p-6 b-rd-6 font-size-mlg font-bold`}>
        </IconFont>
