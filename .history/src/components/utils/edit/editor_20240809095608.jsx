...
const Tiptap = ({ content, editor = {}, provider, saveTimeInit }) => {
  ...
  const createExport = useCallback(format => () => {
    ...
    editor.chain().export({
      ...
      onExport(context) {
        // 获取文件名或设置默认值
        const filename = storage.getItem("documentName") || "default-filename";
  
        // 将 context 的属性复制到新对象中，添加 filename
        const updatedContext = { ...context, filename };
  
        // 处理错误
        if (updatedContext.error) {
          showErrorToast({ message: updatedContext.error.message });
          setIsLoading(false);
          return;
        }
  
        // 使用 updatedContext 的下载方法
        updatedContext.download();
        setIsLoading(false);
      },
    }).run();
  }, [editor])
  ...
  const contentFive = (<div >
    ...
    <div className='p-6 hover-effect b-rd-6' disabled={editor.isEmpty} onClick={createExport('md')}>导出为md格式文档</div>
  </div>)
