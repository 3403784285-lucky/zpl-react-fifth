...
import Style from '../../../pages/ai/picture.jsx'
import { head } from 'lodash';
import { time } from 'echarts';
const { Header, Footer, Sider, Content } = Layout;
const IconFont = createFromIconfontCN({
  scriptUrl: [
    '//at.alicdn.com/t/c/font_4248113_6w6an6wf3kr.js'
  ],
});
        ...
        <Content className='flex  h-full w-full '>
          ...
          <div className="bg-color-white flex-1">
            ...
            {current == 1 ? (<Extraction editor={editor} />) : (current == 2 ? (<Dictionary />) : (current == 3 ? (<Generation />) : (current == 4 ? (<Polishing editor={editor} fontFamilyH1Value={fontFamilyH1Value}
              ...
              setLineHeightH4Value={setLineHeightH4Value} />) : (current == 5 ? (<MyExample editor={editor} />) : (current == 6 ? (<AiHelper />) : (current == 7 ? (<PassageComment editor={editor} />) : (current==8?<Picture editor={editor} />:<Style editor={editor}/>)))))))}
          </div>
        </Content>
      </div>
    </>
