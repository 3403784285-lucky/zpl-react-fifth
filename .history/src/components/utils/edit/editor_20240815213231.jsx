...
const Tiptap = ({ setIsModalVisible,content, editor = {}, provider, saveTimeInit }) => {
  ...
  const [resultText, setResultText] = useState(''); // 用于保存生成的文本内容
  ...
  const fetchTextContinuation = async (formData) => {
    try {
      ...
      while (true) {
        ...
        setResultText(result); // 动态更新 resultText 状态
        // 实时更新模态框内容
        if (resultModal) {
          resultModal.update({
            content: (
  ...
  const continuationClick = async () => {
    ...
    // 保存模态框实例以便更新内容
    setResultModal(modalInstance);

    // 启动文本生成请求
    fetchTextContinuation(formData);
  };
