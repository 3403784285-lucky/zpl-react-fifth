...
const Tiptap = ({ user, room, content, editor = {} }) => {
  ...
  const polishClick = async (value) => {
    ...
    try {
      ...
      generatingModal.destroy(); // 如果请求出错，也要关闭生成中的模态框
      modal.error({
        title: '请求出错',
        content: '生成文字动画时出现了问题，请稍后再试。',
      });
    }
...
`,
  ...
  const handleApplyStylesOften = (fontSize, fontFamily, lineHeight, num) => {
    ...
    if (num != 0) {
      state.doc.descendants((node, pos) => {
        ...
        if (name == 'heading' && level == num) {

          headingPositions.push({ from: pos, to: pos + node.nodeSize });
          console.log(pos, pos + node.nodeSize)
        }
      });
    } else {
      state.doc.descendants((node, pos) => {
    ...
    // 添加 mark 来设置字体大小
    headingPositions.forEach(({ from, to }) => {
      editor.chain().setTextSelection({ from: from, to: to }).setFontFamily(fontFamily).run()
      editor.chain().setTextSelection({ from: from, to: to }).setFontSize(fontSize).run()
      editor.chain().setTextSelection({ from: from, to: to }).setLineHeight(lineHeight).run();
    });
  ...
  const handleApplyStyles = () => {
    handleApplyStylesOften(fontSizeH1Value, fontFamilyH1Value, lineHeightH1Value, 1)
    handleApplyStylesOften(fontSizeH2Value, fontFamilyH2Value, lineHeightH2Value, 2)
    handleApplyStylesOften(fontSizeH3Value, fontFamilyH3Value, lineHeightH1Value, 3)
      ...
      <div className={`position-relative ${shrink ? 'show-no' : ''}`} style={{ width: '35%', backgroundColor: 'red', boxShadow: 'rgba(50, 50, 93, 0.25) 0px 2px 5px -1px, rgba(0, 0, 0, 0.3) 0px 1px 3px -1px' }}>
        ...
        <Content className='flex  h-full w-full '>
          ...
          <div className="bg-color-white flex-1">
            ...
            {current == 1 ? (<Extraction />) : (current == 2 ? (<Dictionary />) : (current == 3 ? (<Generation />) : (current == 4 ? (<Polishing editor={editor} fontFamilyH1Value={fontFamilyH1Value}
              ...
              setLineHeightH4Value={setLineHeightH4Value} />) : (current == 5 ? (<MyExample editor={editor} />) : (current == 6 ? (<AiHelper />) : (current == 7 ? (<PassageComment />) : (<Picture editor={editor} />)))))))}
          </div>
        </Content>
      </div>
    </>
          ...
          <Popover
            ...
            content={(
              <div>
                ...
                <div className='p-6 hover-effect b-rd-6' onClick={() => polishClick("")}>更正式</div>

                <div className='p-6 hover-effect b-rd-6' onClick={() => polishClick("")}>更口语</div>
              </div>
            )}
            onOpenChange={setU}
          >
