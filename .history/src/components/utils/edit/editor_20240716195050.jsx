...
const IconFont = createFromIconfontCN({
  scriptUrl: [
    '//at.alicdn.com/t/c/font_4248113_wqr3hanacf.js'
  ],
});
...
const Tiptap = ({ user, room, content ,editor={}}) => {
  ...
  const saveContent = async () => {
      ...
      const res=await fileFun.update(storage.getItem("documentId"),{content})
...
`,
  ...
  return <>{spin? <Spin size="large" style={{ margin: '300px 520px' }} />:<div style={{ height: '100%' }}>
    ...
    ) : (<>  <div style={{ width: `${shrink?100:75}%`, overflowY: 'auto' }}>
      <EditorContent className={`p-24 ${classes.codeBlock}`} editor={editor} />
    </div>
      <div className={`position-relative ${shrink?'show-no':''}`} style={{ width: '25%', backgroundColor: 'red',boxShadow: 'rgba(50, 50, 93, 0.25) 0px 2px 5px -1px, rgba(0, 0, 0, 0.3) 0px 1px 3px -1px' }}>
      <CaretRightOutlined className='position-absolute font-size-mlg' onClick={toggleSize} style={{left:6,bottom:10}} />
        <Content className='flex  h-full w-full '>
          ...
          <div className="bg-color-white flex-1">
            {current==1?(<Extraction/>):( current==2?(<Dictionary/>):(<Generation/>))}
          </div>
        </Content>
      </div>
    </>
