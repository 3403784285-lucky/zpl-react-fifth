...
const IconFont = createFromIconfontCN({
  scriptUrl: [
    '//at.alicdn.com/t/c/font_4248113_bz4b9e4zmuo.js'
  ],
});
  ...
  return (
    <div className={`upper-frame bg-color-blue flex-center-center shadow`} style={{ height: '8%' }}>
      ...
      <Tooltip placement="bottom" title={"行距"} arrow={mergedArrow}>
        <Popover placement="bottom" content={icons.map((icon, index) => {if(index==0) return <IconFont type={`icon-${icon}`} key={icon} onClick={() => hide(index)} className={`m-10 font-size-mlg font-bolder`}>
        </IconFont>
