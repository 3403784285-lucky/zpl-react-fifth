...
import { decrypt } from '../../../utils/code.js';
import userFun from '../../../api/user/user.js';
  ...
  const editor = useEditor({
    extensions: [
      TableOfContents.configure({
        ...
        onUpdate(content) {
          setItems(content)
          console.log("变化" + content)
          // receiveChild(items,editor,2)
        },
      }),
      Color.configure({
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (userId) {
        ...
        content=res.data.document.content // 获取内容
  ...
  return <div style={{ height: '100%' }}>
    ...
    <div className='flex' style={{ height: '84%', }} > {spin ? (
      ...
      <div style={{ width: '25%', backgroundColor: 'red' }}>
        <Content className='flex shadow h-full w-full'>
          <Menu
            mode="inline"
            items={itemThrees}
