...
const MenuBar = ({ provider,handleSizeChange,lineHeight isFormatBrushActive,emptyFormat, fontSize, fontFamily, shrink, changeShrink, handleFontChange, editor, selectColorComplete, contentPop, contentPopCopy, contentPopThree, currentIcon, currentTextAlign, colorS, open, handleOpenChange, openCopy, handleOpenChangeCopy, openThree, handleOpenChangeThree, openFour, handleOpenChangeFour, applyLineHeight, handleFormatBrushClick, handleFormatBrushDoubleClick }) => {
    ...
    <div className={`upper-frame bg-color-blue flex-end-center shadow p-x-10`} style={{ height: '8%' }}>
      ...
      <Tooltip placement="bottom" title={"行距"} arrow={mergedArrow}>
        <Popover placement="bottom" content={icons.map((icon, index) => {
          ...
          <IconFont type={`icon-${lineHeight??currentIconCopy}`} onClick={applyLineHeight} className='m-4 p-10 b-rd-4 font-size-mlg' />
