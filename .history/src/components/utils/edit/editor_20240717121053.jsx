...
const IconFont = createFromIconfontCN({
  scriptUrl: [
    '//at.alicdn.com/t/c/font_4248113_9q7oi6zq9p8.js'
  ],
});
...
const Tiptap = ({ user, room, content, editor = {} }) => {
  ...
  const itemThrees = [
    ...
    {
      key: 5,
      icon: <IconFont type='//at.alicdn.com/t/c/font_4248113_7fnnv3u936.js' className='font-size-lg'></IconFont>,
      title: '我的素材'
    },
  ]
  ...
  return (<>{spin ? (<Spin size="large" style={{ margin: '300px 520px' }} />) : (<div style={{ height: '100%' }}>
    ...
    <div className='flex' style={{ height: '84%', }} > {!content ? (
      ...
      <div className={`position-relative ${shrink ? 'show-no' : ''}`} style={{ width: '35%', backgroundColor: 'red', boxShadow: 'rgba(50, 50, 93, 0.25) 0px 2px 5px -1px, rgba(0, 0, 0, 0.3) 0px 1px 3px -1px' }}>
        <CaretRightOutlined className='position-absolute font-size-mlg' onClick={toggleSize} style={{ left: 6, bottom: 10 }} />
        <Content className='flex  h-full w-full '>
          ...
          <div className="bg-color-white flex-1">
            ...
            {current == 1 ? (<Extraction />) : (current == 2 ? (<Dictionary />) : (current == 3 ? (<Generation />) : (current==4?(<Polishing fontFamilyH1Value={fontFamilyH1Value}
              ...
              setLineHeightH4Value={setLineHeightH4Value} />):<MyExample/>)))}
          </div>
        </Content>
      </div>
    </>
