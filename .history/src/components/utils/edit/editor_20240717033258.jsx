...
`,
  ...
  const handleApplyStyles = () => {
    const headingNodes = editor.getNodes('heading').filter(node => node.attrs.level === 1);

    // 遍历所有标题一节点，设置样式
    editor.chain().focus();
    headingNodes.forEach(node => {
      editor.chain().setNode('heading', {
        ...node.attrs,
        style: `font-family: ${fontFamilyH1Value}; font-size: ${fontSizeH1Value}; line-height: ${lineHeightH1Value};`, // 设置你的样式
      });
    });
    editor.run();
    console.log("开始应用了")
