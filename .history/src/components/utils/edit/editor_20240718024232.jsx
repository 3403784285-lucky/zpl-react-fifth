          ...
          resultModal.update({
            content: (
              <div className="result-content">
                {text.slice(0, currentIndex + 1).split('').map((char, index) => (
                  <span key={index} style={{ color: 'blue' }}>{char}</span> // 这里设置彩色流动效果
                ))}
              </div>
            )
          });
    ...
    try {
      ...
      const resultModal = modal.info({
        ...
        content: (
          <div className="result-content">
            {/* 使用状态来控制逐字显示 */}
            <ResultAnimation content={res.data} />
          </div>
        ),
        okText: '替换',
        onOk() {
