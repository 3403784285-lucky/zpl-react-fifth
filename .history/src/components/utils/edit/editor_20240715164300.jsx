...
const Tiptap = ({ user, room, content, receiveChild }) => {
  ...
  const createChartClick = async () => {
    ...
    try {
      ...
      setChartData(res.data); // Assuming the API returns chart data
      setModalVisible(true); // Show modal after chart data is fetched
    } catch (error) {
      ...
      <div className='shadow flex-c-center-start' style={{
        ...
        <div className='flex-r-center-center m-4 b-rd-6 hover-effect' style={{ justifyContent: 'space-between' }}>
          <Popover
            ...
            onOpenChange={() => setUniqueOpenCopy(true)}
          >
