  ...
  return (
    <div className={`upper-frame bg-color-blue flex-center-center shadow`} style={{ height: '8%' }}>
      ...
      <Tooltip placement="bottom" title={"行距"} arrow={mergedArrow}>
        <Popover placement="bottom" content={icons.map((icon, index) => {
         ...
         return <IconFont type={`icon-${icon}`} key={icon} onClick={() => hide(index)} className={` p-6 b-rd-6 hover-effect font-size-mlg`}>
          ...
          <IconFont type={`icon-${currentIconCopy}`} onClick={applyLineHeight} className='p-6 b-rd-6 hover-effect font-size-mlg' />

        </Popover>
      </Tooltip>
      <Tooltip placement="bottom" title={"撤回"} arrow={mergedArrow}>
        <IconFont type="icon-undo" onClick={() => editor.chain().focus().undo().run()} className='p-6 b-rd-6 hover-effect font-size-lg' />

      </Tooltip>
      <Tooltip placement="bottom" title={"取消撤回"} arrow={mergedArrow}>

        <IconFont type="icon-redo" onClick={() => editor.chain().focus().redo().run()} className='p-6 b-rd-6 hover-effect font-size-lg' />
      </Tooltip>
      <Tooltip placement="bottom" title={"格式刷(用法:选中要复制的格式点击格式刷，显示选中后，再选中另一处要应用的文本点击格式刷即可)"} arrow={mergedArrow}>

        <IconFont type="icon-painter" onClick={handleFormatBrushClick} onDoubleClick={handleFormatBrushDoubleClick} className='p-6 b-rd-6 hover-effect font-size-lg' />
      </Tooltip>
      ...
      <Tooltip placement="bottom" title={"字体颜色"} arrow={mergedArrow}>
        <IconFont type="icon-font-color" className='p-6 b-rd-6 hover-effect font-size-lg' style={{ color: colorS }} />
      </Tooltip>
      ...
      <Tooltip placement="bottom" title={"标题级别"} arrow={mergedArrow}>
        ...
        <Popover placement="bottom" content={contentPop} trigger="click" open={open} onOpenChange={handleOpenChange}>
          <IconFont type={`icon-${currentIcon}`} className='p-6 b-rd-6 hover-effect font-size-lg' />
        </Popover>
      </Tooltip>
      ...
      <Tooltip placement="bottom" title={"对齐"} arrow={mergedArrow}>
        ...
        <Popover placement="bottom" content={contentPopThree} trigger="click" open={openThree} onOpenChange={handleOpenChangeThree}>
          <IconFont type={`icon-text-align-${currentTextAlign}`} className='p-6 b-rd-6 hover-effect font-size-lg' />
        </Popover>
      </Tooltip>
      ...
      <Tooltip placement="bottom" title={"表格"} arrow={mergedArrow}>
        ...
        <Popover placement="bottom" content={contentPopCopy} trigger="click" open={openCopy} onOpenChange={handleOpenChangeCopy}>
          <IconFont type="icon-table" className='p-6 b-rd-6 hover-effect font-size-lg' />
        </Popover>
      </Tooltip>
      <Tooltip placement="bottom" title={"图像"} onClick={addImage} arrow={mergedArrow}>

        <IconFont type="icon-image" className='p-6 b-rd-6 hover-effect font-size-lg' onClick={() => editor.chain().focus().setTextAlign('right').run()} />
      </Tooltip>
