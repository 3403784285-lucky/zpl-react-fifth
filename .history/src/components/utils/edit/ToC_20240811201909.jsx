...
export const ToCItem = ({ item, onItemClick, isCollapsed, toggleCollapse, collapsedItems }) => {

  const itemStyle = {
    paddingLeft: `${(item.level - 1) * 10}px`, // 控制层级之间的微小缩进，确保视觉上的竖直排列
  };

  return (
    <div
      className={`toc-item level-${item.level} ${item.isScrolledOver ? 'is-scrolled-over' : ''}`}
      style={itemStyle} // 应用样式
    >
        ...
        <div className="toc-children">
          {item.children.map((childItem) => (
            <ToCItem
              ...
              isCollapsed={collapsedItems[childItem.id]}
              toggleCollapse={toggleCollapse}
              collapsedItems={collapsedItems} // 传递 collapsedItems
            />
          ))}
        </div>
      )}
