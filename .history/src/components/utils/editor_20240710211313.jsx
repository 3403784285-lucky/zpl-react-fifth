...
const Tiptap = ({ user, room }) => {
const storage=useStorage()
  const ydoc = useMemo(() => new Y.Doc(), []);   // 使用 useMemo 保证只创建一次 provider
  const provider = useMemo(() => new WebsocketProvider(`ws://192.168.43.214:8081/ws/${room}`, 'tttt', ydoc), [room, ydoc]);
  const [editor, setEditor] = useState({})
  


  useEffect(() => {
    const content=storage.getItem("document").content;
  ...
  setEditor(editorNew)
    ...
    const handleTransaction = () => {
      const { selection } = editorNew.state;
    ...
    editorNew.on('transaction', handleTransaction);

    return () => {
      editorNew.off('transaction', handleTransaction);
    };
  }, [editorNew]);
