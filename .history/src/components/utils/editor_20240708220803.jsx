...
const Tiptap = () => {
  ...
  const contentGenerationClick = async () => {
    ...
    // 创建FormData对象
    const formData = new FormData();
    formData.append('title', '基于XX技术的计算机系统性能优化研究');
    formData.append('project', selectedText);
    formData.append('paper_type', selectedText);
    formData.append('directory_type', selectedText);
    const res = await aiFun.titleGeneration(formData)
