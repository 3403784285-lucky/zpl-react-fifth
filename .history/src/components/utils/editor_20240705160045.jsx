...
const Tiptap = () => {
  ...
  const handleOpenChangeCopy = useCallback((newOpenCopy) => {
    console.log("表格开关");
  ...
  }, [formatToApply, editor, handleFormatBrushClick]);

  const hide = useCallback((index) => {
    console.log("选择开关");
    console.log(index);
  ...
  const hideCopy = useCallback((index) => {
    console.log("选择开关2");
    console.log(index);
    editor.chain().focus().toggleHeading({ level: index }).run();
    setCurrentIcon(icons[index]);
  ...
  const selectColorComplete = useCallback((colorO) => {
    console.log("选择颜色开关");
    console.log("我现在选择的颜色是" + colorO.metaColor.originalInput);
    setColorS((colorS) => {
      colorS = colorO.metaColor.originalInput;
  ...
  }, [editor, colorS, lineHeight, applyLineHeight, selectColorComplete, hide, handleOpenChange, open, openCopy, handleOpenChangeCopy, currentIcon]) 
