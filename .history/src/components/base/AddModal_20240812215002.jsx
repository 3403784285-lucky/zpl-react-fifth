import React, { useEffect, useState } from 'react';
import { Modal, Input, List, message } from 'antd';
import { DownOutlined, createFromIconfontCN } from '@ant-design/icons';
import fileFun from '../../api/user/file';
import { useStorage } from 'web-localstorage-plus';
import toolFun from '../../api/user/toolList';
import { getItem } from 'yjs';

const IconFont = createFromIconfontCN({
    scriptUrl: [
        '//at.alicdn.com/t/c/font_4248113_6bsmqu142h.js'
    ],
});
const TodoModal = ({ visible, setVisible ,setTodoData}) => {
    const [todoName, setTodoName] = useState('');
    const [fileList, setFileList] = useState([]);
    const [selectedFileId, setSelectedFileId] = useState(null);
    const storage=useStorage()


    const handleOk = async() => {
        const res=await toolFun.add({listName:todoName,documentId:selectedFileId})
        if(res,code==200){
            message.success("代办项创建成功")
            const newItem = {
                listId: todoData.length + 1, // 假设使用数组长度作为新的 ID
                userId: storage,getItem("openid"), // 假设当前用户 ID 为 1，你可以根据实际情况调整
                listName: todoName,
                isCompleted: 0 // 新增的待办项默认未完成
              };
            
              // 更新 todoData 状态，新增的待办项添加到数组的末尾
              setTodoData([...todoData, newItem]);
            

        }
        setVisible(false);
    };


    const handleTodoNameChange = (e) => {
        setTodoName(e.target.value);
    };
    const fetchData = async () => {
        const res=await fileFun.getAllUserDocument(storage.getItem("openid"))
        setFileList(res.data)

    }
    useEffect(() => {
        fetchData()

    })

    const handleFileClick = (file) => {
        console.log(`File clicked: ${file.name}`);
        setSelectedFileId(file.id); // 设置被点击的文件ID
        // Handle file click action here
    };

    return (
        <>
            <Modal
                title="创建待办项"
                open={visible}
                onOk={handleOk}
                onCancel={() => setVisible(false)}
            >
                <Input
                    className='m-t-10'
                    placeholder="待办名称，下面选择文件表示想要通过该代办想跳转到对应文件（可选）"
                    value={todoName}
                    onChange={handleTodoNameChange}
                />
                <List
                    style={{height:'50vh',overflowY:'auto'}}

                    itemLayout="horizontal"
                    className='m-t-10'
                    dataSource={fileList}
                    renderItem={file => (
                        <List.Item onClick={() => handleFileClick(file)} className={selectedFileId === file.id ? 'is-selected p-10' : 'p-10'} >
                            <List.Item.Meta
                                avatar={<IconFont type="icon-wordIcon" className="font-size-lg" />}
                                title={file.name}
                            />
                        </List.Item>
                    )}
                />
            </Modal>
        </>
    );
};

export default TodoModal;
