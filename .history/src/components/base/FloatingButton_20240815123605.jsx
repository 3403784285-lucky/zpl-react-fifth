...
const FloatingButton = () => {
    ...
    useEffect(() => {
        ...
        fetchData()
        // 读取保存的位置
        let es = localStorage.getItem('buttonPosition'); // 确保使用 localStorage 而不是 storage
        if (es) {
            const savedPosition = JSON.parse(es);
            if (savedPosition) {
    ...
    useEffect(() => {
        return storage.setItem('buttonPosition', JSON.stringify(position));
    }, [])
    ...
    return (
        <Draggable onDrag={handleDrag} bounds={bounds} position={position}>
