  ...
  useEffect(() => {
    const fetchData=async()=>{
        ...
        if(res4.code==200){
            ...
            const yesData = data.find(item => item.timeCategory === "昨天")
            setSales(todayData.totalSales)
            setCounts(todayData.pointsOrderCount+todayData.membershipOrderCount)
            setSalesPer(((todayData.membershipOrderCount - ) / yesterdayAmount) * 100)
            setCountsPer( ((todayOrder - yesterdayOrder) / yesterdayOrder) * 100)
        }
    }
