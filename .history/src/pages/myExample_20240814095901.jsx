import { Radio, Card, Modal, Button, Tag, Dropdown, Skeleton, message } from 'antd';
...
function MyExample({ editor }) {
    ...
    const handleDeleteExample=async(id)=>{
        setLoading1(true)
        const res=await elementFun.delete(id);
        if(res.code==200){
    ...
    const onClick = (item) => {
        ...
        editor.chain().insertContentAt(currentEditor.state.selection.anchor, {
            type: 'image',
            attrs: {
              src:item.content,
            },
    ...
    const handleExample = (e, item) => {
        if (e.key == 100) {
            modal.confirm({
                title: "编辑素材",
                icon: <IconFont type='icon-jihebiaoshi21'></IconFont>,
                content: <div className='flex-c-center-center'><UploadMaterialForm isMod={true} item={item} closeModal={closeModal} /></div>,
                footer: null,
                closable: true
            });
            ...
            modal.confirm({
                title: "删除素材",
                icon: <IconFont type='icon-jihebiaoshi21'></IconFont>,
                content: <div className='flex-c-center-center m-10'>你确定删除此素材？</div>,
                onOk:()=>handleDeleteExample(item.id),
            });
        }
    }
    const handleSelect = (e) => {
        modal.confirm({
            title: "上传素材",
            icon: <IconFont type='icon-jihebiaoshi21'></IconFont>,
            content: <div className='flex-c-center-center'><UploadMaterialForm isMod={true} closeModal={closeModal} /></div>,
            footer: null,
            closable: true
    ...
    return <Dropdown
        ...
        trigger={['contextMenu']}
    ><div className="item-frame p-10 " style={{ height: '84vh', overflowY: 'auto' }} >
            {contextHolder}
            {loading1 ? <Skeleton /> : data.map((item, index) => (
                <Dropdown
                    menu={{
                        items: items1,
                        onClick: (e) => handleExample(e, item)
                    }}
                    trigger={['contextMenu']}
                >  <Card
                    ...
                    key={item.key}
                    className="item b-rd-6 m-x-18 m-y-10 flex-c-center-center shadow"
                    hoverable={true}
                    style={{
                        width: 300,
                        height: 250
