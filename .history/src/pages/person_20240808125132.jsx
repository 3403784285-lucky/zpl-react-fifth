...
const PersonalCenter = () => {
    ...
    const handleNicknameBlur = async () => {
        ...
        const res = await userFun.update(inputValue, avatar, storage.getItem("openid"))
        if (res.code == 200) {
        ...
        <div className='position-relative' style={{ height: '250px', backgroundImage: "url('/img/person.png')", backgroundSize: 'cover' }}>
            <div className="float-card position-absolute flex-r-start-center p-50" style={{ left: 33, bottom: -160, width: '78vw' }}>
                <div className=' p-x-20'>
                    ...
                    <div className='font-size-mlg m-t-10 m-l-10 flex-c-center-center'>
                        {isEditingNickname ? (
                            <Input
                                value={inputValue}
                            ...
                            <Tooltip title="点击编辑昵称">
                                <span onClick={() => {setIsEditingNickname(true);setInputValue(nickname)}}>
                                    {nickname}
                                </span>
                            </Tooltip>
                        )}{storage.getItem("user").level==1?<Tag className='m-l-6 m-t-6' color="cyan">尊贵会员   {storage.getItem("user").level==1?storage.getItem("user").memberExpireTime:''}</Tag>:''}
                    </div>
                </div>
