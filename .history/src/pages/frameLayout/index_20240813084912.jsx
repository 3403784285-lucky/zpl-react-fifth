...
const itemTwosCopy = [
  ...
  getItem('recycle-bin', <QuestionCircleFilled />, '价格表'),
  getItem('recycle-bin', <QuestionCircleFilled />, '文档管理')
]
const items = [
  {
    type: 'group',
...
const Index = () => {
  ...
  const [status,setStatus]=useState(storage.getItem("user").userRole)
    ...
    <Layout >
      <Header className='p-10' style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'white' }} >
        <div className="pre-frame flex-r-start-end">
          ...
          {status&&<Search className='m-l-80' placeholder="请输入关键字"
