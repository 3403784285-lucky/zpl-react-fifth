...
const Index = () => {
  ...
  const toSmall=()=>{
    dispatch(setSearchText(searchTerm)); // 调用action设置搜索内容
    ...
    <Layout >
      <Header className='p-10' style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'white' }} >
        ...
        <Search className='m-l-80' placeholder="请输入关键字"
                    value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{width:'300px'}} onClick={onSearch} enterButton />
        </div>
