...
function File() {
    ...
    useEffect(() => {
        const fetchData = async (page) => {
            ...
            // 进行请求，传入页码和每页条数
            const res = await fileFun.getAllUserDocument(userId, page, pageSize);
            console.log(res.data);

            // 更新数据和总条数
            setDataDeal(res.data);
            setTotal(res.total); // 假设后端返回总条数
        };
    ...
    return <div className="h-full p-10" >
        ...
        <Pagination className='m-t-20 position-absolute' style={{ bottom: 40, left: 500 }} defaultCurrent={1} pageSize={5} total={total} showSizeChanger showQuickJumper hideOnSinglePage={true} />
