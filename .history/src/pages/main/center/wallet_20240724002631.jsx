...
const Wallet = () => {
    ...
    const [isMember, setIsMember] = useState(storage.getItem("isBuying"))
    const [num, setNum] = useState(0);
    const [description, setDescription] = useState('');
    useEffect(() => {
        
        (isMember)?setIsMember(true):setIsMember(false);
    },[])
            ...
            <Spin spinning={loading}>
                <Card style={{ minWidth: 360 }}>
                    <ProCard type={"inner"} headerBordered bordered tooltip={"用于平台AI问答"} title={isMember?<strong>尊享会员</strong>:<strong>我的钱包</strong>}>
                        <strong>{isMember?'时间：':'积分 :'} </strong><span style={{ color: "red", fontSize: 18 }}>{isMember?'无':storage.getItem("user").money}</span>
                    </ProCard>
                    <br />
                    <Card type={"inner"} title={<strong>{isMember?'会员':'积分'}商城 💰️</strong>}>
                        ...
                        <ProCard style={{ marginTop: -20 }} layout={"center"}>
                            <span>本商品为虚拟内容,用于平台接口调用,购买后不支持<strong style={{ color: "red" }}>退换  最近支付宝正在维护可能会造成支付失败</strong>。确认支付表示您已阅读并接受用户协议 。如付款成功后10分钟后未到账，请联系站长微信：lzh15116575485
                            </span>
                        </ProCard>
                    </Card>
                    <br />
