...
const OrderTable = () => {
    ...
    const handleOk = async (order, fun,a) => {
        const res = await fun(order.id)
        if (res.code == 200) {
            if(a=='c')
            message.success("订单取消成功")
           else   message.success("订单删除 成功")
           
            const r = await orderFun.getUserAllOrder()
            setDataDeal(r.data)
        }
    }
    const cancelView = (order) => {
        Modal.confirm({
            ...
            okText: '确认',
            cancelText: '取消',
            onOk: () => handleOk(order, orderFun.cancelOrder,'c')
        });
    }
    ...
    const handleDelete = (order) => {
        Modal.confirm({
            ...
            okText: '确认',
            cancelText: '取消',
            onOk: () => handleOk(order, orderFun.deleteOrder,'d')
        });
    };
    const seeView = (record) => {
        setCurrent(record)
            ...
            render: (text, record) => (
                <>
                    ...
                    {(record.status === '已取消' || record.status === '已完成') && (
                        <Button onClick={() => seeView(record)}>查看</Button>
                    )}
                ...
                {dataDeal && (
                    <div className="order-details">
                        <div className="order-row">
                            ...
                            <div className="order-value">{current.id}</div>
                        </div>
                        <div className="order-row">
                            <div className="order-label">过期时间:</div>
                            <div className="order-value">{current.endOrderTime}</div>
                        </div>

                        <div className="order-row">
                            <div className="order-label">订单金额 (元):</div>
                            <div className="order-value">{current.amount}</div>
                        </div>
                        <div className="order-row">
                            <div className="order-label">订单状态:</div>
                            <div className="order-value">{current.status}</div>
                        </div>
                        <div className="order-row">
                            <div className="order-label">创建时间:</div>
                            <div className="order-value">{current.orderTime}</div>
                        </div>
                        <div className="order-row">
                            <div className="order-label">商品类别:</div>
                            <div className="order-value">{current.orderType}</div>
                        </div>
                        <div className="order-row">
                            <div className="order-label">商品描述:</div>
                            <div className="order-value">{current.description}</div>
                        </div>
                        <Button type="primary" danger style={{ marginTop: '20px' }} onClick={() => handleDelete(current)}>
                            删除订单
                        </Button>
                    </div>
                )}
