...
import { Dropdown, Popover, Popconfirm, Avatar, Typography, Space,List,Button, Input, Modal, message } from 'antd'
...
function Favorites() {
    ...
    const handleLogClick = async (e, folder) => {
        e.stopPropagation();
        const res = await foldersFun.getFolderLog(folder.folderId)
        if (res.code == 200) {
            setLogs(res.data)
        ...
        Modal.confirm({
            title: "操作日志",
            icon: <IconFont type='icon-jihebiaoshi21'></IconFont>,
            content: <div style={{height:'60vh',overflowY:'auto'}}>
                 <List
                    itemLayout="horizontal"
                    dataSource={logs.length>0?logs:res.data}
                    renderItem={(operation) => (
                      <List.Item onClick={()=>toDocument(operation)}>
                        <List.Item.Meta
                          avatar={<Avatar src={operation.userAvatar} />}
                          title={
                            <div>
                              <Text>{operation.operation}</Text>
                              <Text style={{ marginLeft: "10px" }}>{operation.operationTime}</Text>
                            </div>
                          }
                          description={operation.operation=='创建文件夹成功'||
                            <Space>
                              <IconFont type='icon-wordIcon'/>
                              <Text ellipsis={true} delete={operation.operation=='删除'}>{operation.documentName}</Text>
                            </Space>
                          }
                        />
                      </List.Item>
                    ...
                    >
                        ...
                        <Popover
                            ...
                            content={<div>
                                ...
                                <div className='p-6 hover-effect b-rd-6' onClick={(e) => handleLogClick(e, folder)}>操作日志</div>
                            </div>}
