...
const OperationItem = ({ operation }) => {
    return (
        <div style={{ display: "flex", marginBottom: "10px" }}>
            <img
                src={operation.userAvatar}
                alt="User Avatar"
                style={{ width: "50px", height: "50px", borderRadius: "50%" }}
            />
            <divn className="m-t-10" style={{ marginLeft: "10px" }}>
                <div>{operation.operationTime}</div>
                <div>{operation.operation}</div>
                <div style={{height:80}}><IconFont type='icon-wordIcon' className='font-size-mlg m-r-10'/>{operation.documentName}</div>
            </divn>
        </div>
    );
};
