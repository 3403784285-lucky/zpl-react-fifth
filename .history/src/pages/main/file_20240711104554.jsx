...
function File() {
    ...
    const start = async (e) => {
        const start = async () => {
            ...
            // 收集选中项的 ID
            const selectedIds = selectedRowKeys.map(key => dataDeal.find(item => item.key === key).id);
            console.log('选中的 ID:', selectedIds);

            // 在这里执行批量删除或其他操作
            // 示例：
            // const res = await deleteDocumentBatch(selectedIds);
            ...
            render: (text, record, index) => {
                ...
                return (
                    <span className='flex-start-center'>
                        ...
                        <div onClick={() => toDocument(record)}>{isEditing ? (
                            <Input
    ...
    return <div className="h-full p-10" >
        ...
        <Table columns={columns}   rowKey={(record) => record.key}   rowSelection={rowSelection} loading={isLoading} dataSource={currentData} onChange={onChange} pagination={false} rowClassName="hover-row className='position-relative'"
