...
function BigEditor() {
    ...
    const [contentCopy, setContentCopy]=useState(0)
...
const location=useLocation()
    ...
    const generateItems = () => {
        // 这里假设文档内容 contentCopy 是 Markdown 格式，提取标题作为目录项
        
    };

    const itemsCopy = generateItems();
    useEffect(()=>{
            ...
            const fetchData = async () => {
              ...
              if (userId) {
                ...
                setContentCopy(); // 获取内容
                const regex = /^#\s+(.+)/gm; // 匹配 Markdown 标题
        const matches = [...res.data.document.content.matchAll(regex)];
        const itemsCopy = matches.map((match, index) => ({
            id: index,
            title: match[1], // 提取标题内容
        }));
        
                setItems(res.data.document.content)
              }
            };
            fetchData();
    },[])
    ...
    return <div className="flex h-full shadow">
        ...
        <div className="" style={{ height: '100%', width: '80%' }}><Tiptap editor={editor} user={user} room={3} ydoc={ydoc} content={contentCopy}/></div>
