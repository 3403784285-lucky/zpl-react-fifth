

import { Radio, Dropdown, Typography, Space, Button, Divider, Popover, message } from 'antd';
import { DownOutlined, createFromIconfontCN } from '@ant-design/icons';
import foldersFun from '../../../api/user/folders';
import { encrypt } from '../../../utils/code';
import { useStorage } from "web-localstorage-plus";
import { Table, Modal, Pagination } from 'antd';
import fileFun from '../../../api/user/file';
import { storeFile } from '../../../utils/code';
const IconFont = createFromIconfontCN({
    scriptUrl: [
        '//at.alicdn.com/t/c/font_4248113_6bsmqu142h.js'
    ],
});

const items2 = [
    {
        key: '1',
        label: '今天',
    },
    {
        key: '2',
        label: '昨天',
    },
    {
        key: '3',
        label: '近七天',
    },
    {
        key: '4',
        label: '更早',
    },
];
const items1 = [
    {
        key: '5',
        label: '所有人',
    },
    {
        key: '6',
        label: '我自己',
    },
    {
        key: '7',
        label: '其他人',
    },

];
const items3 = [
    {
        key: '8',
        label: 'word',
    },

];
import { Select, Tag } from 'antd';
import TableMy from '../../../components/utils/main/tableMy';
import shareCFun from '../../../api/user/share';
import { useNavigate } from 'react-router-dom';

const options1 = [
    {
        key: 0,
        value: '普通文档',
    },
    {
        key: 1,
        value: '论文',
    },
];
const options3 = [
    {
        key: 8,
        value: 'word文档',
    },
    {
        key: 9,
        value: 'md文档',
    },
];
const options2 = [
    {
        key: '理科',
        value: '理科',
    },
    {
        key: '文科',
        value: '文科',
    },
];
const tagRender = (props) => {
    const { label, value, closable, onClose } = props;
    console.log(label, value)
    const onPreventMouseDown = (event) => {
        event.preventDefault();
        event.stopPropagation();
    };
    return (
        <Tag
            color={'green'}
            onMouseDown={onPreventMouseDown}
            closable={closable}
            onClose={onClose}
            style={{
                marginInlineEnd: 4,
            }}
        >
            {label}
        </Tag>
    );
};
// const getFolders = async () => {
//     const res = await userFun.getFolders();
//     console.log(res + "结束")
// }
function FileLay() {
    const navigate = useNavigate()
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedOption, setSelectedOption] = useState('文档');
    const [selectedType, setSelectedType] = useState(0)
    const [isImport, setIsImport] = useState(false)
    const storage = useStorage()
    const [selectedLabel, setSelectedLabel] = useState("理科")
    const importRef = useRef(null)
    const onClick = async () => {
        // const res=await fileFun.favorite({userId:3,documentId:61});
        // const res = await userFun.searchUsers()
        // console.log(res);
        setIsModalOpen(true);
    }
    const handleOptionClick = (option) => {
        setSelectedOption(option);
        if(option=='导入'){
            setIsImport(true)
            handleImportClick()

        }else {
            setIsImport(false)

        }
    };
    const handleOk = () => {
        setIsModalOpen(false);
    };
    const handleCancel = () => {
        setIsModalOpen(false);
    };
    const createDocument = async () => {
        const category = selectedOption;//文档还是模板
        const userId = storage.getItem("openid")//用户id
        const type = selectedType;
        const subject = selectedLabel;
        let res;
        if(isImport){
            res = await fileFun.create({ userId, type, subject, category, content: '默认文本' ,name:storage.getItem("documentName")})
        }else
         res = await fileFun.create({ userId, type, subject, category, content: '默认文本' })
        console.log(res)
        if (res) {
            message.success(res.msg)
            handleOk()
            navigate(`/big-editor?` + encrypt(res.data.id))
        }
    }
    const labelSelect = (value, option) => {
        console.log(value, option)
        setSelectedLabel(option.value)
    }
    const documentSelect = (value, option) => {
        console.log(value, option)
        selectedType(option.key)
    }
    //导入文档
    const handleImportClick = useCallback(() => {
        importRef.current.click()
    }, [])
    const handleImportFilePick = async(e) => {
        const file = e.target.files[0]
        if (file) {
            const reader = new FileReader(); // 创建 FileReader 对象
    
            reader.onload = (event) => {
                const fileContent = event.target.result; // 获取文件内容
                console.log(fileContent); // 输出文件内容到控制台
                // 在这里处理文件内容，例如将其保存到状态或传递到其他函数中
            };
    
            reader.onerror = (error) => {
                console.error("Error reading file:", error); // 错误处理
            };
    
            reader.readAsText(file); // 读取文件内容为文本
        }
        importRef.current.value = ''
        if (!file) {
            return
        }
        await storeFile(file);
        storage.setItem("importFile",true)
        storage.setItem("documentName",file.name.replace(/\.[^/.]+$/, ''))
         console.log(file)        
    }
    return <>
        <div className="start-frame p-x-30 p-t-10">
            <div className="upper-frame p-t-20">
                <div className="title font-size-lg font-bold">开始</div>
                <div className="lay-out m-16 flex">
                    <div className="list-one b-rd-10 m-10 p-10 flex-r-center-center shadow" style={{ width: '17%', height: '70px' }}>
                        <IconFont className='font-size-vlg m-r-20' type="icon-xinzengwenjianjia" />
                        <div className="right-add-file flex-c-center-center">
                            <div className="big-text" onClick={onClick}>文档</div>
                            <div className="small-text font-size-sm text-color-grey">暂时只有word文档</div>
                        </div>
                    </div>
                    <Modal title="新建" visible={isModalOpen} footer={null} onOk={handleOk} onCancel={handleCancel} width={300}>
                        <div className="flex">
                            <div className="model m-r-10 p-x-10 p-y-6 b-rd-6" style={{ backgroundColor: selectedOption == '文档' ? '#c7d2d3' : 'white' }} onClick={() => handleOptionClick('文档')}>
                                <IconFont className='font-size-vlg ' type="icon-wendang" />
                                <p>文档</p>
                            </div>
                            <div className="model m-r-20 p-x-10 p-y-6 b-rd-6" style={{ backgroundColor: selectedOption == '导入' ? '#c7d2d3' : 'white' }} onClick={() => handleOptionClick('导入')}>
                                <IconFont className='font-size-vlg' type="icon-shangchuan" />
                                <input onChange={handleImportFilePick} accept=".md,.docx"  type="file" ref={importRef} style={{ display: 'none' }} />
                                <p>导入</p>
                            </div>
                        </div>
                        <h4 className='m-t-10 m-b-6'>文档类型</h4>
                        <Select
                            maxTagCount={1}
                            placeholder="暂时只能导入word和md文档哦"
                            mode="tags"
                            onSelect={documentSelect}
                            tagRender={tagRender}
                            style={{ width: '100%' }}
                            maxCount={1}
                            options={isImport?options3:options1}
                        />
                        <h4 className='m-t-10 m-b-6'>学科类型</h4>
                        <Select
                            maxTagCount={1}
                            mode="tags"
                            onSelect={labelSelect}
                            tagRender={tagRender}
                            maxCount={1}
                            defaultValue={'理科'}
                            style={{ width: '100%' }}
                            options={options2}
                        />
                        <Button className='w-full m-y-10 b-rd-16' type='primary' onClick={createDocument}>确认</Button>
                        <Button className='w-full m-y-5 b-rd-16 bg-color-grey text-color-white' onClick={handleCancel}>取消</Button>
                    </Modal>

                    <div className="list-two b-rd-10 m-10 p-10 flex-r-center-center shadow" style={{ width: '17%', height: '70px', }}>
                        <IconFont className='font-size-vlg m-r-20' type="icon-mobanzhongxin-01" />
                        <div className="module-center flex-c-center-center">
                            <div className="big-text">模版中心</div>
                            <div className="small-text font-size-sm text-color-grey">从模版中获取灵感</div>
                        </div>

                    </div>

                </div>

                <div className="bottom-frame position-relative" style={{ height: '58vh' }}>
                    <div className="title font-size-lg font-bold">最近文档</div>

                    <div className="dropdown-frame position-absolute" style={{ right: 0, top: 65 }}>
                        <Dropdown
                            className='m-r-10'
                            menu={{
                                items: items3,
                                selectable: true,

                            }}
                        >
                            <Typography.Link>
                                <Space>
                                    类型
                                    <DownOutlined />
                                </Space>
                            </Typography.Link>
                        </Dropdown>
                        <Dropdown
                            className='m-r-10'
                            menu={{
                                items: items1,
                                selectable: true,

                            }}
                        >
                            <Typography.Link>
                                <Space>
                                    归属
                                    <DownOutlined />
                                </Space>
                            </Typography.Link>
                        </Dropdown>
                        <Dropdown
                            className='m-r-10'

                            menu={{
                                items: items2,
                                selectable: true,

                            }}
                        >
                            <Typography.Link>
                                <Space>
                                    时间
                                    <DownOutlined />
                                </Space>
                            </Typography.Link>
                        </Dropdown>
                    </div>
                    <div className='h-full'>
                        <TableMy getFun={shareCFun.recent} dataContent={3} deleteFun={foldersFun.deleteRecentDocument} deleteBatchFun={null} uniqueText="记录" />
                    </div>

                </div>
            </div>
        </div>
    </>
}
export default FileLay;



