...
function FileLay() {
    ...
    const createDocument = async () => {
        ...
        const res = await fileFun.create({ userId, type, subject, category, content: '默认文本' })
        console.log(res)
        if (res) {
                    ...
                    <Modal title="新建" visible={isModalOpen} footer={null} onOk={handleOk} onCancel={handleCancel} width={300}>
                        <div className="flex">
                            ...
                            <div className="model m-r-20 p-x-10 p-y-6 b-rd-6" style={{ backgroundColor: selectedOption == '导入' ? '#c7d2d3' : 'white' }} onClick={() => handleOptionClick('导入')}>
                                ...
                                <input onChange={handleImportFilePick} type="file" ref={importRef} style={{ display: 'none' }} />
                                <p>导入</p>
                            </div>
                        </div>
                        ...
                        <Select
                            ...
                            options={options2}
                        /> */}
                        <Button className='w-full m-y-10 b-rd-16 m-t-30' type='primary' onClick={createDocument}>确认</Button>
