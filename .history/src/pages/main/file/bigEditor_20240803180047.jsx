...
function BigEditor() {
  ...
  useEffect(() => {
    ...
    providerRef.current = new WebsocketProvider(`ws://192.168.50.41:8081/ws/${room}`, 'tttt', ydoc);

    // Update CollaborationCursor extension with new provider
    editor.chain().configure({ CollaborationCursor: { provider: providerRef.current } }).focus().run();
