...
function BigEditor() {
  ...
  // console.log("连接")
  const provider = useMemo(() => new WebsocketProvider(`ws://192.168.50.41:8081/ws/${room}`, 'tttt', ydoc), [room, ydoc]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    websocketRef.current = new WebsocketProvider(`ws://192.168.50.41:8081/ws/${room}`, 'tttt', ydoc);

    // Update CollaborationCursor extension with new provider
    editor.chain().configure({ CollaborationCursor: { provider: websocketRef.current } }).focus().run();
