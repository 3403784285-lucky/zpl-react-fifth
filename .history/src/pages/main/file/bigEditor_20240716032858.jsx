...
if(editor&&contentCopy!="?"){
  ...
  // 设置过滤条件（如果有）
  const filter = []; // 根据实际需要设置过滤条件

  // 设置选项（如果有）
  const options = {}; // 根据实际需要设置选项

  // 调用 getHierarchicalIndexes 函数来获取层级内容数组
  const content = getHierarchicalIndexes(state.doc, filter, options);

  // 更新目录内容，可以使用 TableOfContents 的 onUpdate 方法
  TableOfContents.onUpdate(content);
