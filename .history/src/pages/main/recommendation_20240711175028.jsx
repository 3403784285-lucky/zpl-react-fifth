    ...
    return <div className="p-36" style={{ height: '80.5vh', overflowY: "auto" }}>
        ...
        <div className="doc m-b-10">
            ...
            <div className="item-frame p-10 flex" style={{ justifyContent: 'space-between' }}>
                {data1.map(item => (
                    <div key={item.id} className="item b-rd-6 shadow p-10 m-10 flex-c-center-center" style={{ width: '24%', height: '250px' }}>
                        {item.type === '图片' && (
                            <img className="m-b-10" src={item.content} alt={item.name} style={{ width: '90%', height: '78%' }} />
                        )}
                        <div className="title font-bold">
                            {/* 假设你要在标题前添加一个图标 */}
                            <IconFont className='font-size-mlg' type='icon-wordIcon' />&nbsp;&nbsp;
                            {item.name}
                        </div>
                    </div>
                ))}
