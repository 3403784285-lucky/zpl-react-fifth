


const items=[
    {
        key:1,
        url:"",
        title:"",
        time:""
    },    {
        key:2,
        url:"",
        title:"",
        time:""
    },    
    {
        key:3,
        url:"",
        title:"",
        time:""
    },
]
let myDom=items.map((item,)=>{
    return <>
    
    </>
})
function Recommendation()
{
    return <div className="p-10">
        <div className="first-title font-bold font-size-lg m-10">
            文档模板
        </div>
        <div className="first-content">
            <div className="item-frame p-30 flex" style={{justifyContent:'space-between'}} >
                <div className="item b-rd-6 shadow p-10 m-10 flex-c-center-center" style={{width:'28%',height:'250px'}}>
                    <img src="data:image/jpeg;base64," alt="图片示例" style={{width:'80%',height:'100px'}} />
                    <div className="title font-bold">入党申请书</div>
                    <div className="pres font-size-sm text-color-grey text-overflow-1">2024-12-10 23:12</div>
                </div>
                <div className="item b-rd-6 shadow p-10 m-10 flex-c-center-center" style={{width:'28%'}}>
                    <img src="data:image/jpeg;base64," alt="图片示例" style={{width:'80%',height:'100px'}} />
                    <div className="title font-bold">入党申请书</div>
                    <div className="pres font-size-sm text-color-grey text-overflow-1">2024-12-10 23:12</div>
                </div>
                <div className="item b-rd-6 shadow p-10 m-10 flex-c-center-center" style={{width:'28%'}}>
                    <img src="data:image/jpeg;base64," alt="图片示例" style={{width:'80%',height:'100px'}} />
                    <div className="title font-bold">入党申请书</div>
                    <div className="pres font-size-sm text-color-grey text-overflow-1">2024-12-10 23:12</div>
                </div>
            </div>

        </div>
    
    
    
    </div>
}
export default Recommendation