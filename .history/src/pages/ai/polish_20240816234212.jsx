  ...
  const handleTransaction = () => {
    ...
    if ($from.sameParent($to)) {
      ...
      if (parent.type.name === 'heading') {
        ...
        if (level === 1) {
          updateStyles(fontFamilyH1Value, fontSizeH1Value, lineHeightH1Value, parent.marks);
        } else if (level === 2) {
          updateStyles(fontFamilyH2Value, fontSizeH2Value, lineHeightH2Value, parent.marks);
        } else if (level === 3) {
          updateStyles(fontFamilyH3Value, fontSizeH3Value, lineHeightH3Value, parent.marks);
        }
      } else if (parent.type.name === 'paragraph') {
        updateStyles(fontFamilyH4Value, fontSizeH4Value, lineHeightH4Value, parent.marks);
      }
    }
  };

  const updateStyles = (defaultFontFamily, defaultFontSize, defaultLineHeight, marks) => {
    const familyMark = marks.find(mark => mark.attrs.fontFamily);
    const sizeMark = marks.find(mark => mark.attrs.fontSize);
    const lineHeightMark = marks.find(mark => mark.attrs.lineHeight);

    setFontFamily(familyMark ? familyMark.attrs.fontFamily : defaultFontFamily);
    setFontSize(sizeMark ? sizeMark.attrs.fontSize : defaultFontSize);
    setLineHeight(lineHeightMark ? lineHeightMark.attrs.lineHeight : defaultLineHeight);
  };
