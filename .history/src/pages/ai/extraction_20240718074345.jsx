...
import api from '../../api/user/ai'; // Assuming you have an API module set up
...
const Extraction = () => {
  const [current, setCurrent] = useState('ocr'); // Default selection
  ...
  const handleChange = async ({ fileList: newFileList }) => {
    ...
    if (newFileList.length > 0) {
      ...
      try {
        ...
        switch (current) {
          case 'ocr':
            response = await api.ocr(formData);
            break;
          case 'asr':
            ...
            response = await api.asr(formData);
            break;
          case 'table':
            // Assuming table recognition API endpoint is `ocrTable`
            response = await api.ocrTable(formData);
            break;
          default:
            break;
        }
  ...
  return (
    <div className='p-30 w-full'>
      ...
      <div className='flex-c-center-center'>
        ...
        {loading && <LoadingOutlined style={{ fontSize: 24, color: '#1890ff' }} spin />}
        {previewImage && (
