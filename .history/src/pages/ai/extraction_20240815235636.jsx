...
const Extraction = ({ editor }) => {
    ...
    const parseMarkdownTable = (markdown) => {
        const rows = markdown
          .trim()
          .split('\n')
          .filter(row => !row.includes('---')) // 过滤掉分隔符行
          .map(row => row.trim().split('|').slice(1, -1).map(cell => cell.trim() || ' ')); // 确保每个单元格有内容
        ...
        const tableContent = {
          ...
          content: rows.map((row, rowIndex) => ({
            ...
            content: row.map((cell, cellIndex) => ({
              type: rowIndex === 0 ? 'tableHeader' : 'tableCell', // 第一行作为表头
              content: [{ type: 'text', text: cell }], // 确保内容是文本节点
            })),
          })),
        };
    ...
    const handleCopyText = () => {
        if (current != "table") {
            ...
            const tableContent = parseMarkdownTable(recognitionResult.text);
            editor.commands.insertContent(tableContent);
        }
    };
