...
function Polishing({ editor, handleApplyStyles, handleResetStyles, fontFamilyH1Value, setFontFamilyH1Value, fontFamilyH2Value, setFontFamilyH2Value, fontFamilyH3Value, setFontFamilyH3Value, fontFamilyH4Value, setFontFamilyH4Value, fontSizeH1Value, setFontSizeH1Value, fontSizeH2Value, setFontSizeH2Value, fontSizeH3Value, setFontSizeH3Value, fontSizeH4Value, setFontSizeH4Value, lineHeightH1Value, setLineHeightH1Value, lineHeightH2Value, setLineHeightH2Value, lineHeightH3Value, setLineHeightH3Value, lineHeightH4Value, setLineHeightH4Value }) {
  ...
  const parseEditorContent = () => {
    ...
    state.doc.descendants((node) => {
      if (node.type.name === 'heading' || node.type.name === 'paragraph') {
        const level = node.attrs.level || null;
        const styleAttrs = extractStylesFromNode(node);

        if (level === 1) {
          setFontFamilyH1Value(styleAttrs.fontFamily || fontFamilyH1Value);
          setFontSizeH1Value(styleAttrs.fontSize || fontSizeH1Value);
          setLineHeightH1Value(styleAttrs.lineHeight || lineHeightH1Value);
        } else if (level === 2) {
          setFontFamilyH2Value(styleAttrs.fontFamily || fontFamilyH2Value);
          setFontSizeH2Value(styleAttrs.fontSize || fontSizeH2Value);
          setLineHeightH2Value(styleAttrs.lineHeight || lineHeightH2Value);
        } else if (level === 3) {
          setFontFamilyH3Value(styleAttrs.fontFamily || fontFamilyH3Value);
          setFontSizeH3Value(styleAttrs.fontSize || fontSizeH3Value);
          setLineHeightH3Value(styleAttrs.lineHeight || lineHeightH3Value);
        } else if (node.type.name === 'paragraph') {
          setFontFamilyH4Value(styleAttrs.fontFamily || fontFamilyH4Value);
          setFontSizeH4Value(styleAttrs.fontSize || fontSizeH4Value);
          setLineHeightH4Value(styleAttrs.lineHeight || lineHeightH4Value);
        }
      }
    });
  };
  ...
  const extractStylesFromNode = (node) => {
    ...
    if (node.marks && node.marks.length > 0) {
      node.marks.forEach((mark) => {
        ...
        if (mark.type.name === 'lineHeight') {
          lineHeight = mark.attrs.lineHeight;
        }
      });
    }
    // 备用方案：从 node.attrs.style 获取样式
    if (node.attrs.style) {
      fontFamily = node.attrs.style.match(/font-family: ([^;]+)/)?.[1] || fontFamily;
      fontSize = node.attrs.style.match(/font-size: ([^;]+)/)?.[1] || fontSize;
      lineHeight = node.attrs.style.match(/line-height: ([^;]+)/)?.[1] || lineHeight;
    }
  ...
  useEffect(() => {
    if (editor) {
      // 获取编辑器内容
      const content = editor.getJSON(); // 假设使用 editor.getJSON() 方法获取内容
      parseEditorContent(content); // 解析内容并设置样式
    }
  }, [editor]);
