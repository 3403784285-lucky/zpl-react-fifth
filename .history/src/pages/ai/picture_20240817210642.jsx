...
const Picture = ({ editor }) => {
    ...
    let markmapInstance = null; // 保存 Markmap 实例
    ...
    const handleMarkdownChange = async () => {
      ...
      try {
        const response = await fetch('https://f504iccf72ke3bha.aistudio-hub.baidu.com/mindMap', {
          method: 'POST',
          body: formData,
          headers: {
        ...
        const processStream = async () => {
          ...
          // 如果 Markmap 实例已存在，进行增量更新
          if (markmapInstance) {
            markmapInstance.setData(root); // 增量更新数据
            markmapInstance.fit(); // 调整布局
          } else {
            // 第一次渲染时创建 Markmap 实例
            const container = document.getElementById('markmap-container');
            container.innerHTML = '';
            markmapInstance = Markmap.create(container, null, root);
          }
          // 控制数据流动速度
          setTimeout(processStream, 500); // 控制更新频率S`
        };
        processStream(); // 开始处理流
