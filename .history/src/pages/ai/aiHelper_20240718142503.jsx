...
const AiHelper = () => {
    ...
    // Function to add a message to the state with animation effect
    ...
    const handleSend = () => {
        if (inputValue || fileList.length) {
            const newMessage = {
                ...
                files: fileList,
                sender: 'user',
                animation: true, // Flag to enable animation effect
            };
            ...
            addMessageWithAnimation(pendingMessage);
...
本文探讨了文心一言大模型的研究背景、理论基础、设计实践、性能评估及未来展望。通过深入的理论分析和实证研究，展示了模型的特点与创新，并指出了当前挑战与潜在应用领域，为相关领域的研究和应用提供了有价值的参考。`,
                ...
                addMessageWithAnimation(answeredMessage);
            }, 2000);
    ...
    useEffect(() => {
        // Function to handle animation effect
        const animateMessage = (message) => {
            const duration = 50; // Animation duration per character (milliseconds)
            const animatedMessage = { ...message, animatedContent: '' };
            let index = 0;
            const interval = setInterval(() => {
                animatedMessage.animatedContent += message.content.charAt(index);
                setMessages((prevMessages) =>
                    prevMessages.map((msg) => (msg === message ? { ...animatedMessage } : msg))
                );
                index++;
        ...
        // Check for messages that need animation
        messages.forEach((message) => {
            if (message.animation && !message.animatedContent) {
                    ...
                    <List.Item style={{ justifyContent: item.sender === 'user' ? 'flex-end' : 'flex-start' }}>
                        <div className={`message-box ${item.sender === 'user' ? 'user-message' : 'assistant-message'}`}>
                            <Typography.Text className={item.status === 'pending' ? 'pending-text' : ''}>
                                {item.animatedContent || item.content}
                            </Typography.Text>
