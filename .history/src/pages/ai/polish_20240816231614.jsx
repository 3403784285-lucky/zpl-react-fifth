  ...
  const parseEditorContent = (content) => {
    ...
    content.content.forEach((node) => {
      if (node.type === 'heading') {
        ...
        const { fontFamily, fontSize, lineHeight } = extractStylesFromNode(node);

        switch (level) {
          case 1:
            setFontFamilyH1Value(fontFamily || fontFamilyH1Value);
            setFontSizeH1Value(fontSize || fontSizeH1Value);
            setLineHeightH1Value(lineHeight || lineHeightH1Value);
            break;
          case 2:
            setFontFamilyH2Value(fontFamily || fontFamilyH2Value);
            setFontSizeH2Value(fontSize || fontSizeH2Value);
            setLineHeightH2Value(lineHeight || lineHeightH2Value);
            break;
          case 3:
            setFontFamilyH3Value(fontFamily || fontFamilyH3Value);
            setFontSizeH3Value(fontSize || fontSizeH3Value);
            setLineHeightH3Value(lineHeight || lineHeightH3Value);
            break;
          default:
            break;
        }
        ...
        const { fontFamily, fontSize, lineHeight } = extractStylesFromNode(node);
        setFontFamilyH4Value(fontFamily || fontFamilyH4Value);
        setFontSizeH4Value(fontSize || fontSizeH4Value);
        setLineHeightH4Value(lineHeight || lineHeightH4Value);
      }
    });
  };

  const extractStylesFromNode = (node) => {
    const fontFamily = node.attrs?.style?.match(/font-family: ([^;]+)/)?.[1];
    const fontSize = node.attrs?.style?.match(/font-size: ([^;]+)/)?.[1];
    const lineHeight = node.attrs?.style?.match(/line-height: ([^;]+)/)?.[1];
